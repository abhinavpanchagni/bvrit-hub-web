  ...
  if (!data) {
    return (
      <main className="flex min-h-screen items-center justify-center">
        <h1 className="text-5xl font-black">Subject Not Found</h1>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[#f6f6f6]">
      <section className="border-b-4 border-black bg-blue-600 text-white">
        <div className="mx-auto max-w-7xl px-6 py-16">
          <Link
            href="/semester-1"
            className="rounded-xl border-2 border-black bg-white px-5 py-2 font-black text-gray-700 shadow-[4px_4px_0_#000]"
          >
            ← Back
          </Link>

          <p className="mt-8 inline-block rounded-full border-2 border-black bg-yellow-300 px-5 py-2 font-black text-gray-700">
            {data.code}
          </p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {data.branches.map((branch) => (
              <span
                key={branch}
                className="rounded-lg border-2 border-black bg-white px-4 py-2 font-black text-gray-700"
              >
                {branch}
              </span>
            ))}
      ...
      <section className="mx-auto max-w-6xl px-6 py-16">
        {data.units.map((unit, index) => {
          ...
          return (
            <div
              key={unit}
              className="mb-6 overflow-hidden rounded-3xl border-4 border-black bg-white shadow-[8px_8px_0_#000]"
            >
              <button
                onClick={() =>
              ...
              {open === index + 1 && (
                <div className="grid gap-5 p-8 md:grid-cols-2 lg:grid-cols-5">
                  ...
                  <a
                    href={unitData?.notes || "#"}
                    target="_blank"
                    className={`rounded-2xl border-2 border-black p-6 text-center text-xl font-black ${
                      unitData?.notes
